...
-(void)setLeftViewController:(UIViewController *)leftViewController{
    ...
    _leftViewController.view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [self menuWidth], ScreenHeight)];
...
-(void)setRightViewController:(UIViewController *)rightViewController{
    ...
    //提前设置ViewController的viewframe，为了懒加载view造成的frame问题，所以通过setter设置了新的view
    _rightViewController.view = [[UIView alloc] initWithFrame:CGRectMake([self emptyWidth], 0, [self menuWidth], ScreenHeight)];
    //自定义View需要主动调用viewDidLoad
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (point.x <= actionWidth || point.x > ScreenWidth - actionWidth) {
            return YES;
        } else {
            return NO;
        }
...
- (void)showLeftViewControllerAnimated:(BOOL)animated {
    ...
    [UIView animateWithDuration:[self animationDurationAnimated:animated] animations:^{
        _rootViewController.view.center = CGPointMake(_rootViewController.view.bounds.size.width/2 + self.menuWidth, _rootViewController.view.center.y);
        _leftViewController.view.frame = CGRectMake(0, 0, [self menuWidth], ScreenHeight);
...
- (void)showRightViewControllerAnimated:(BOOL)animated {
    ...
    [UIView animateWithDuration:[self animationDurationAnimated:animated] animations:^{
        _rootViewController.view.center = CGPointMake(_rootViewController.view.bounds.size.width/2 - self.menuWidth, _rootViewController.view.center.y);
        _rightViewController.view.frame = CGRectMake([self emptyWidth], 0, [self menuWidth], ScreenHeight);
        _coverView.alpha = MaxCoverAlpha;
    }];
}
...
- (void)updateLeftMenuFrame {
    if (!self.animationType) {
        _leftViewController.view.center = CGPointMake(CGRectGetMinX(_rootViewController.view.frame)-CGRectGetWidth(_rootViewController.view.frame)/2+self.emptyWidth/2, _leftViewController.view.center.y);
    }else{
...
- (void)updateRightMenuFrame {
    if (!self.animationType) {
        //        _rightViewController.view.frame = CGRectMake(ScreenWidth+CGRectGetMinX(_rootViewController.view.frame), 0, [self menuWidth], ScreenHeight);
        _rightViewController.view.center = CGPointMake(ScreenWidth + CGRectGetMaxX(_rootViewController.view.frame)-CGRectGetWidth(_rootViewController.view.frame)/2-self.emptyWidth/2, _rightViewController.view.center.y);
    }else{
        _rightViewController.view.center = CGPointMake((ScreenWidth + CGRectGetMaxX(_rootViewController.view.frame))/2, _rightViewController.view.center.y);
    }
}

//菜单宽度
- (CGFloat)menuWidth {
    return MenuWidthScale * ScreenWidth;
}

//空白宽度
- (CGFloat)emptyWidth {
    return ScreenWidth - self.menuWidth;
}
